...
export const alt = "JEE Maths chapter practice questions";
export const size = OG_SIZE;
export const contentType = OG_CONTENT_TYPE;

export default async function Image({ params }) {
  const { chapter: chapterSlug } = await params;
  const chapters = await getChapterTaxonomy("Mathematics").catch(() => []);
  const match = chapters.find((c) => c.slug === chapterSlug);

  return brandedOgImage({
    eyebrow: "JEE Maths",
    title: match ? match.chapter : "Maths Practice Questions",
    subtitle: match
