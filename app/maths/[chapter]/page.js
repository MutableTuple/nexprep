...
// DB stores the subject as "Mathematics"; the site's user-facing label/URL
// prefix is "Maths" (matching app/maths/page.js).
const SUBJECT = "Mathematics";
const SUBJECT_LABEL = "Maths";
const SUBJECT_SLUG = "maths";
...
export async function generateMetadata({ params }) {
  ...
  const title = `${match.chapter} — JEE Maths Practice Questions`;
  const description = `Practice ${match.count} JEE Main & Advanced Maths question${
    match.count === 1 ? "" : "s"
...
export default async function Page({ params }) {
  ...
  const jsonLd = {
    ...
    "@type": "CollectionPage",
    name: `${match.chapter} — JEE Maths Practice Questions`,
    url: canonicalUrl,
    breadcrumb: {
      "@type": "BreadcrumbList",
      itemListElement: [
