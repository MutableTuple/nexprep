...
export async function generateMetadata({ params }) {
  ...
  const title = `${match.chapter} — JEE Physics Practice Questions`;
  const description = `Practice ${match.count} JEE Main & Advanced Physics question${
    match.count === 1 ? "" : "s"
...
export default async function Page({ params }) {
  ...
  const jsonLd = {
    ...
    "@type": "CollectionPage",
    name: `${match.chapter} — JEE Physics Practice Questions`,
    url: canonicalUrl,
    breadcrumb: {
      "@type": "BreadcrumbList",
      itemListElement: [
