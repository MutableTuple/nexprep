...
// userId   → whose friends we're showing (the profile being viewed)
// viewerId → the currently logged-in user (may be null)
// isOwn    → true when viewerId === userId, unlocks search + requests + remove
export default function FriendsTab({ userId, viewerId, isOwn }) {
  ...
  async function loadAll() {
    ...
    setLoading(true);
    try {
      const tasks = [listFriendships(userId, "accepted")];
      if (isOwn) tasks.push(listPendingFriendRequests(userId));
      const [accepted, incoming] = await Promise.all(tasks);
      setFriends(accepted);
      if (isOwn) setPending(incoming ?? []);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {isOwn ? (
        ...
        <FriendsList loading={loading} friends={friends} userId={userId} />
      )}
    </div>
  );
}